<?php
    ...
    if (is_login()){
        ...
        if ($_SESSION['user_id'] == 'admin' && $_SESSION['is_admin']==1){
            ...
            echo "<script> alert('관리자 전용 입니다.') </script>";
            echo "<script> location.href = './index.php'  </script>";
            //echo "<script>잘못된 접근입니다.</script>";
...
function validatePassword($password){
	...
	//		$errMSG = "잘못된 패스워드";
		...
		{
                   try{
			$stmt = $con->prepare('INSERT INTO users(username, password, userprofile, salt, end_date) VALUES(:username, :password, :userprofile, :salt, :enddate)');
			$stmt->bindParam(':username',$username);
...
<form id="form" method="post" enctype="multipart/form-data" class="form-horizontal" style="margin: 0 100px 0 100px;border: solid 1px #999;border-radius:4px;padding:30px">
	<table class="table table-responsive" style="margin:0">
    <tr>
        <? $r1 = md5(rand().microtime(TRUE)); ?>
    ...
    <tr>
        <? $r4 = md5(rand().microtime(TRUE)); ?>
    	<td><label class="control-label">이름</label></td>
        <td><input class="form-control" type="text" name="<? echo $r4; ?>" placeholder="이름을 입력하세요" autocomplete="off" readonly 
    onfocus="this.removeAttribute('readonly');" />
    ...
    <tr>
        <? $r4 = md5(rand().microtime(TRUE)); ?>
        <td><label class="control-label">사용기간</label></td>
        <td>
