...
function Calendarize() {
	...
	return {
		...
		buildYearCalendar: function(el, year) {
			...
			$("#calendar").append("<div id='year'>"+year+"</div>");
		...
		buildMonth: function(monthNum, year, opts) {
			...
			daysInMonth.forEach(function(c, d) {
				...
				var ddd = moment(dateParsed).format('YYYY-MM-DD');
				if(data.includes(ddd)){				
					//alert(ddd);
 				   $dayNode.classList.add('holiday'); 				   
				}

				var ddd1 = moment(dateParsed).format('YYYY-MM-DD');
				if(data_w.includes(ddd1)){									
 				   $dayNode.classList.add('xdate'); 				   
				}
